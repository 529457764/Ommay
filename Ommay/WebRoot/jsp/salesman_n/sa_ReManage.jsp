...
<%--
			...
			<!-- 右侧主要内容 -->
			<div class="col-xs-12 col-sm-10 col-md-10 col-lg-10" id="showBox" data-box="sa_ReManage">
				<div class="row col-md-12" style="margin-bottom:10px;">
					<a href="<%=path%>/view/sa_VipManage"><button class="btn btn-default">会员管理</button></a>
					<a href="<%=path%>/view/sa_ReManage"><button class="btn btn-primary">客户代表管理</button></a>
				</div>
				<div class="row col-md-12">
					<a href="<%=path%>/view/sa_register1"><button class="btn btn-default btn-sm">添加客户代表</button></a>
				</div>
	...
	<script type="text/javascript">
		$(function(){
			$.ajax({
				url:"<%=path%>/account/flagUser/getFlagDate?flag=4",
				type:"post",
				success:function(data){
					...
					for(var i=0; i<result.length; i++) {
						...
						str +='<td>'+result[i].accountGroup+'</td>';
						if(i%2 == 0) {
							str +='<td>男</td>';
						} else {
							str +='<td>女</td>';
